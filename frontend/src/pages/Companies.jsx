...
const Companies = () => {
  ...
  // Filter logic, mirrors InternshipFilter's applyFilters pattern
  const applyFilters = (data) => {
    ...
    if (filters.branch)
      result = result.filter((c) =>
        c.eligibilityCriteria?.allowedBranches?.includes(filters.branch)
      );

    if (filters.sort === "ctc-high") {
      result.sort((a, b) => b.ctc - a.ctc);
    }
    if (filters.sort === "ctc-low") {
      result.sort((a, b) => a.ctc - b.ctc);
    }
  ...
  const displayedData = applyFilters(companies);

  if (loading) {
    return (
      <div className="min-h-screen flex justify-center items-center text-lg font-semibold bg-[#f4fbf7]">
        Loading Companies...
      </div>
    );
  }
      ...
      <nav className="bg-white shadow-md px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-1">
            ...
            <Link
              to="/companies"
              className="flex items-center gap-2 px-4 py-2 rounded-full font-medium bg-[#1FAA59] text-white shadow-lg shadow-[#1FAA59]/25"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button className="relative group flex items-center gap-3 p-1 pr-3 rounded-full hover:bg-gray-100 transition-all duration-300">
            ...
            <div className="hidden sm:flex flex-col items-start">
              ...
              <span className="text-xs text-gray-500">Placement Cell</span>
            </div>
      ...
      <main className="flex-grow px-4 py-10">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-center text-gray-500 mb-6">
            Manage placement drives, company information, and recruitment details.
          </p>

          <div className="flex justify-center mb-10">
            <button
              onClick={handleAdd}
              className="flex items-center gap-2 bg-[#006838] hover:bg-[#00552e] text-white px-6 py-2.5 rounded-full text-sm font-semibold shadow transition-all duration-200"
            >
              <Plus size={18} />
              Add Company
            </button>
          ...
          <AdminCompanyFilter filters={filters} setFilters={setFilters} />

          {/* Cards */}
          {displayedData.length === 0 ? (
            companies.length === 0 ? (
              <p className="text-center text-gray-500 mt-10">
                No companies found. Click "Add Company" to create your first placement drive.
              </p>
            ) : (
              <p className="text-center text-gray-500 mt-10">
