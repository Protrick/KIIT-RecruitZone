...
const Jobs = () => {
  ...
  // ===== Data from backend (was: `import jobs from "../data/jobs.json"`) =====
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        setError("Couldn't load jobs right now. Please try again in a moment.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const applyFilters = (jobsList) => {
    ...
    if (filters.sort === "ctc-high") {
      filtered.sort((a, b) => parseFloat(b.ctc) - parseFloat(a.ctc));
    }

    if (filters.sort === "ctc-low") {
      filtered.sort((a, b) => parseFloat(a.ctc) - parseFloat(b.ctc));
    }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f4fbf7]">
      ...
      <section className="text-center py-10">
        <h1 className="text-3xl font-bold text-[#006838]">Job Opportunities</h1>
        <p className="text-gray-600 mt-2">Training & Placement Cell – KIIT University</p>
      </section>
        ...
        <div className="flex bg-white shadow-md rounded-full p-1 border border-green-200">
          <button
            ...
            className={`px-6 py-2 rounded-full text-sm font-semibold transition-all duration-200
              ${tab === "latest" ? "bg-[#006838] text-white shadow" : "text-[#006838] hover:bg-green-100"}`}
          >
            Latest Jobs
          </button>
          ...
          <button
            onClick={() => setTab("all")}
            className={`px-6 py-2 rounded-full text-sm font-semibold transition-all duration-200
              ${tab === "all" ? "bg-[#006838] text-white shadow" : "text-[#006838] hover:bg-green-100"}`}
          >
            All Jobs
          </button>
        </div>
      ...
      {/* ===== JOB CARDS ===== */}
      {loading ? (
        <p className="text-center text-gray-500 py-14">Loading jobs...</p>
      ) : error ? (
        <p className="text-center text-red-500 py-14">{error}</p>
